...
public class MainActivity extends AppCompatActivity implements SignInFragment.LoginInteractionListener {
    ...
    public void login(String userId, String pwd) {
        ...
        if (networkInfo != null && networkInfo.isConnected() && credentialCheck(userId,pwd)) {
            mIntent = new Intent(this, HomeActivity.class);
            RegisteryTask task = new RegisteryTask();
            task.execute(buildString(userId, pwd, LOGIN_URL));
...
//        finish();
    ...
    public void register(String e, String pw){
        ConnectivityManager manager = (ConnectivityManager) getSystemService(Context.CONNECTIVITY_SERVICE);
        NetworkInfo networkInfo = manager.getActiveNetworkInfo();

        if (networkInfo != null && networkInfo.isConnected() && credentialCheck(e,pw)) {
            mIntent = new Intent(this, HomeActivity.class);

            RegisteryTask task = new RegisteryTask();
            task.execute(buildString(e, pw, REG_URL));
        } else {
            Toast.makeText(this, "No network connection available. Cannot provide services",
                    Toast.LENGTH_LONG).show();
        }
